...
<s:if test='rtp=="N"'>
    <tr>
     <td>
      ...
      <td><b>UniProt:</b>
   <s:iterator value="record.xrefList.xref.{?#this.ns=='UniProt'}" id="ext" status="status">
      ...
      <br />
      <b>RefSeq:</b>
   <s:iterator value="record.xrefList.xref.{?#this.ns=='RefSeq'}" id="ext" status="status">
      <s:if test="#status.last">
         <t:insertDefinition name="externallink"/>
      </s:if>
      <s:else>
...
<s:elseif test='rtp=="E"'>
    <tr>
     <td>
      <b><s:property value="record.partList.part.size"/> Interactors:</b>

  <s:if test="record.partList.part.size>0">
<!--   <ol class="int_list">
   <s:iterator value="record.partList.part" id="mol" status="status">
      <s:url id="dipmolUrl" includeParams="none" action='record'>
      ...
      <s:set name="common_org" value="#mol.node.xrefList.xref.{?#this.typeAc=='dxf:0007'}[0].node.name"/>
   ...
   <s:else>
    <li class="int_list">
     ...
     from 
      ...
      <s:else>
         <b><i><s:property value="#mol.node.xrefList.xref.{?#this.typeAc=='dxf:0007'}[0].node.label"/></i></b>; <!-- species -->
      </s:else>
...
<s:elseif test='rtp=="X"'>
    <tr>
     <td>
   <s:set name="int_ac" value="record.xrefList.xref.{?#this.typeAc=='dxf:0021'}[0].ac"/>
   <s:url id="dipintUrl" includeParams="none" action='record'>
   ...
   <s:iterator value="record.xrefList.xref.{?#this.typeAc=='dxf:0015'}" id="ext" status="status">
      <s:if test="#status.last">
         <t:insertDefinition name="externallink"/>
      </s:if>
      <s:else>
...
<s:elseif test='rtp=="S"'>
    <tr>
     <td>
      <b>Title:</b> <s:property value="record.attrList.attr.{?#this.ac=='dip:0004'}[0].value.value"/>
      <br />
      <b>Authors:</b> <s:property value="record.attrList.attr.{?#this.ac=='dip:0010'}[0].value.value"/>
      <br />
      <b>Citation:</b> <i><s:property value="record.attrList.attr.{?#this.ac=='dip:0009'}[0].value.value"/></i> <!-- journal -->
       <s:property value="record.attrList.attr.{?#this.ac=='dip:0013'}[0].value.value"/>, <!-- year -->
       <b><s:property value="record.attrList.attr.{?#this.ac=='dip:0011'}[0].value.value"/></b>(<s:property value="record.attrList.attr.{?#this.ac=='dip:0012'}[0].value.value"/>): <!-- volume and issue -->
       <s:property value="record.attrList.attr.{?#this.ac=='dip:0015'}[0].value.value"/> <!-- pages -->
      <br />
      <b>PubMed:</b>
   <s:iterator value="record.xrefList.xref.{?#this.typeAc=='dxf:0009'}" id="ext" status="status">
      <s:if test="#status.last">
         <t:insertDefinition name="externallink"/>
      </s:if>
      <s:else>
