...
<table width="100%" cellspacing="0" class="dstable" border="1">
 ...
 <s:iterator value="tabData.group" var="grp" status="gpos" > <!-- groups -->
  <s:iterator value="#grp" var="entry" status="epos" > <!-- groups -->
   <tr>
    <s:iterator value="tabData.colFld" var="cell" status="cpos" >     <%-- columns --%>
      ...
      <s:if test="#epos.count==1">
       ...
       <s:if test="tabData.colFld[#cpos.index]=='sources'">         <%-- dataset column --%>
        ...
        <s:elseif test="tabData.colFld[#cpos.index]=='unc'">   <%-- uncompressed column --%>
         [<A HREF="ftp://dip.mbi.ucla.edu/dip/imex<s:property value="tabData.data[#entry]['ftp-path']"/><s:property value="tabData.data[#entry]['file']"/>.gz">FTP</A>]&nbsp;
         [<A HREF="file?ds=<s:property value="tabData.id"/>&fn=<s:property value="tabData.data[#entry]['file']"/>">HTTP</A>]
        </s:elseif>
        <s:elseif test="tabData.colFld[#cpos.index]=='cmp'">   <%-- compressed column --%>
         [<A HREF="ftp://dip.mbi.ucla.edu/dip/imex<s:property value="tabData.data[#entry]['ftp-path']"/><s:property value="tabData.data[#entry]['file']"/>.gz">FTP</A>]&nbsp;
         [<A HREF="file?ds=<s:property value="tabData.id"/>&fn=<s:property value="tabData.data[#entry]['file']"/>&ff=gz">HTTP</A>]
        </s:elseif>
     ...
     <s:else>
      ...
      <s:else>
       ...
       <s:elseif test="tabData.colFld[#cpos.index]=='unc'">                <%-- uncompressed column --%>
         [<A HREF="ftp://dip.mbi.ucla.edu/dip/imex<s:property value="tabData.data[#entry]['ftp-path']"/><s:property value="tabData.data[#entry]['file']"/>.xml">FTP</A>]&nbsp;
         [<A HREF="file?ds=<s:property value="tabData.id"/>&fn=<s:property value="tabData.data[#entry]['file']"/>.xml">HTTP</A>]
       </s:elseif>
       <s:elseif test="tabData.colFld[#cpos.index]=='cmp'">  <%-- compressed column --%>
         [<A HREF="ftp://dip.mbi.ucla.edu/dip/imex<s:property value="tabData.data[#entry]['ftp-path']"/><s:property value="tabData.data[#entry]['file']"/>.xml.gz">FTP</A>]&nbsp;[<A HREF="file?ds=<s:property value="tabData.id"/>&fn=<s:property value="tabData.data[#entry]['file']"/>.xml&ff=gz">HTTP</A>]
       </s:elseif>
       <s:elseif test="tabData.colFld[#cpos.index]=='file'">                       <%-- file column --%>
         <s:property value="tabData.data[#entry]['file']"/>.xml
       </s:elseif>
