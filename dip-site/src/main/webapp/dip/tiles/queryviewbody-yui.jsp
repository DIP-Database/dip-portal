...
<script>
  ...
  var qt='<s:property value="qt"/>';
  var query = '<s:property value='query'/>';
  var actionname='<s:property value='com.opensymphony.xwork2.ActionContext.name'/>';

  if(qt.indexOf("seq") > -1 || qt.indexOf("motif") > -1 ){

     YAHOO.util.Event.addListener( window, "load",
     YAHOO.mbi.view.query.init( "sequenceQuery?query=" + query 
                                + "&qt=" + qt 
                                + "&rtp=<s:property value="rtp"/>",
                              "<s:property value="sl"/>" ));
   } else {
    YAHOO.util.Event.addListener( window, "load",
    YAHOO.mbi.view.query.init( actionname + "?query=" + query 
                                          + "&qt=" + qt,
                              '<s:property value="sl"/>' ));
   }
