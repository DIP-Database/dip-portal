package edu.ucla.mbi.prl.struts.action;

/* =============================================================================
 * $Id:: PrlRecordAction.java 2887 2013-01-02 16:53:44Z lukasz                 $
 * Version: $Rev:: 2887                                                        $
 *==============================================================================
 *                                                                             $
 * RecordAction action - returns DIP records                                   $
 *                                                                             $
 *     TO DO:                                                                  $
 *         - agent setup                                                       $
 *=========================================================================== */
...
import edu.ucla.mbi.dip.struts.action.*;
...
public class PrlRecordAction extends RecordSupport 
    implements AccessionAware   {

    //--------------------------------------------------------------------------
    // database access (NOTE: should be moved to the model/agent ???)
    //----------------

    DxfService ncbiDbService; // NCBI
    DxfService ebiDbService;  // EBI
    DxfService prlDbService;  // Prolinks
    DxfService dipDbService;  // DIP
    
    public void setNcbiDbService( DxfService service ) {
        this.ncbiDbService = service;
    }
    
    public void setEbiDbService( DxfService service ) {
        this.ebiDbService = service;
    }

    public void setPrlDbService( DxfService service ) {
        this.prlDbService = service;
    }
    ...
    DipAgentConfig dipAgentConf;

    public void setDipAgentConfig( DipAgentConfig conf ) {
        this.dipAgentConf = conf;
    }
    ...
    public String getCounts() throws Exception {
        ...
        if ( getAc() != null && getAc().length() > 0 ) {
            ...
            log.debug("count processed:" + getModelCountList() );
        }
        return "json";
    }
    ...
    */
    ...
    public String buildData() throws Exception {
        ...
        setJqData( new ArrayList() );
        setJqGridData( new HashMap() );
        ...
        if( dxfQuery != null ) {
            ...
            if ( getSummary() == null ) {
                try {
                    ...
                    // NOTE: temporary - ultimately all will move 
                    //       into something like master dbService
                    //----------------------------------------------------------

                    if ( !dxfPrefix.equals( "" ) ) {
                        if ( dxfPrefix.equals( "pmid" ) ) {
                            setSummary( ncbiDbService.getDxfNode( dxfPrefix,
                                                                  getAc(),
                                                                  getDl() ) );
                        }
                        if ( dxfPrefix.equals( "refseq" ) ) {
                            setSummary( ncbiDbService.getDxfNode( dxfPrefix,
                                                                  getAc(),
                                                                  getDl() ) );
                        }
                        if ( dxfPrefix.equals( "uniprot" ) ) {
                            setSummary( ebiDbService.getDxfNode( dxfPrefix,
                                                                 getAc(),
                                                                 getDl() ) );
                        }
                        if ( dxfPrefix.equals( "prl" ) ) {
                            setSummary( prlDbService.getDxfNode( dxfPrefix,
                                                                 getAc(),
                                                                 getDl()) );
                        }
                        if ( dxfPrefix.equals( "dip" ) ) {
                            setSummary( dipDbService.getDxfNode( dxfPrefix,
                                                                 getAc(),
                                                                 getDl() ) );
                        }
                    } else {
                        setSummary( ebiDbService.getDxfNode( "picr", getAc(),
                                                             getDl() ) );
                    }
                    if ( getSummary() != null ) {
            ...
            if ( srvType.equals( "reflist" ) ) {
                ...
                if ( cref == null ) {
                    cref = getModel().getService( getMd(), getSl() )
                        .getDxfRefList( dxfPrefix, dxfQuery, srvTag);
            ...
            if ( getContext().isAgentOn() ) {
                ...
                log.debug( "Action: GetRecord starting DipAgent" );
      
                Map<String,DxfService> dsm = 
                    new HashMap<String,DxfService>();
                
                dsm.put("dipDbService" ,dipDbService);
                dsm.put("ncbiDbService",ncbiDbService);
                dsm.put("ebiDbService" ,ebiDbService);
      
                new Thread(new DipAgent( dipAgentConf, getSummary(), 
                                         getSession(), dsm ) ).start();
            }
