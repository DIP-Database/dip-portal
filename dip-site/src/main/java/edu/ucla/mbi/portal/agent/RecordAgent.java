...
/* =============================================================================
 # $Id:: RecordAgent.java 2712 2012-09-18 23:08:47Z lukasz                     $
 # Version: $Rev:: 2712                                                        $
 #==============================================================================
 #                                                                             $
 # RecordAgent - retrieves asynchroneously data from the record database       $
 #                                                                             $
 #=========================================================================== */
...
public class RecordAgent implements Runnable{
    ...
    public RecordAgent( NodeType record,
                        Map session,
	...
	log = LogFactory.getLog( RecordAgent.class );
    ...
    public RecordAgent( AgentConfig config,
                        NodeType record,
                        Map session
                        //Map<String,DxfService> dsm 
                        ){
	...
	log.info("RecordAgent: run starting...");
	...
	if( session!=null && record!=null ){
		...
		log.info( "RecordAgent: skipping jobs: too many agents per session" );
	    } else {
		log.info( "RecordAgent: processing " + record.getAc() );
		this.process(dac);
		log.info( "RecordAgent: processing " + record.getAc() + "  DONE" );
	    }
	    ...
	    log.info("RecordAgent: done...");
	}
    }

    public void process( AgentConfig conf ){
	...
	log.info( "RecordAgent: recType=" + recType );
	...
	if(slist!=null && slist.size()>0){
	    log.info( "RecordAgent: servicelist=" + slist.size() );
		...
		log.info( "RecordAgent: component-type="+ctp+" component-ns="+cns+" stg="+stg+" rtg="+rtg);
		
		if( ctp.equals("self")){
		    if( record.getNs().toLowerCase().equals(cns.toLowerCase())){
			log.info("RecordAgent: xref: NS="+cns+" AC="+record.getAc());
			...
			if( rcache!=null && lcache!=null ){
				...
				if(result!=null){
				    ...
				    log.info("RecordAgent: part: hit added: "+record.getAc()+"_"+rtg);
				} else {
				    log.info("RecordAgent: part no record found...");
				}
			    } else{
                                log.info("RecordAgent: record already cached.");
                            }
		...
		if(ctp.equals("xref")){
			...
			if(cxref.getNs().toLowerCase().equals(cns.toLowerCase())){
			    log.info("RecordAgent: xref: NS="+cns+" AC="+cxref.getAc());
				...
				if(cvc.equals("getDxfNode") && rcache.get(cxref.getAc()+"_"+rtg)==null){
					...
					log.info("RecordAgent: part: hit added: "+cxref.getAc()+"_"+rtg);
				    } else {
					    log.info("RecordAgent: part no record found...");
				    }
				} else{
		...
		if( ctp.equals("part") && record.getPartList() != null ){
			...
			if( cnd.getNs().equals(cns) ){
			    log.info( "RecordAgent: xref: NS=" + cns + 
                                      " AC="+cnd.getAc());
			    
			    ConcurrentHashMap lcache = null;
			    synchronized( session ){
				lcache = (ConcurrentHashMap) 
                                    session.get("list-cache");
				if( lcache==null ){
				    session.put( "list-cache",
				...
				rcache = (ConcurrentHashMap) 
                                    session.get("record-cache");
			    }
			    
			    if( rcache!=null && lcache!=null ){
				if( cvc.equals("getDxfNode") && 
                                    rcache.get(cnd.getAc()+"_"+rtg)==null ){
				    NodeType result = 

                                        svr.getDxfNode( cnd.getNs().toLowerCase(),
                                                        cnd.getAc(), stg );
				    if( result!=null ){
					rcache.put( cnd.getAc() +"_" + rtg,
                                                    result );
					log.info( "RecordAgent: " + 
                                                  "part: hit added: " + 
                                                  cnd.getAc()+"_"+rtg );
				    } else {
					log.info( "RecordAgent:" +
                                                  " part no record found...");
				    }
				} else{
				    log.info( "RecordAgent:" +
	...
	if(record.getAc().matches(".+\\d+E")){
		...
		log.info("RecordAgent: part: "+cnode.getAc());
		...
		if(rcache!=null){
			...
			if(result!=null){
			    ...
			    log.info("RecordAgent: part: hit added: "+cnode.getAc()+"_base");
			} else {
			    log.info("RecordAgent: part no record found...");
			}
		    } else{
	...
	if(record.getAc().matches(".+\\d+N")){
	    ...
	    log.info("RecordAgent: list: "+record.getAc()+"_E");
		...
		if( lcache.get(record.getAc()+"_E")==null ){
		    List<XrefType> result 
                        = dbservice.getDxfRefList( record.getNs().toLowerCase(),
                                                   record.getAc(), "E" );

		    if( result!=null && result.size()>0 ){
			lcache.put( record.getAc() + "_E", result );
			log.info( "RecordAgent: list: hit added: " 
                                  + record.getAc() + "_E");
		    } else {
			log.info( "RecordAgent: list no record found..." );
		    }
		} else{
		    log.info( "RecordAgent: list already cached." );
		}
	    }
	}
    }
